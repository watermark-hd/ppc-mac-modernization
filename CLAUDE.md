...
スクリプト(`connect-aqualink.bat` / `setup-aqualink.ps1`)を同梱。詳細は
[[project_auth_debugging_next]](Claude Codeのメモリ)参照。
...
2026-08-19、依頼者より提案。現状のWebDAV方式(タスク2b)は「Finderやエクスプローラーに
...
「第2の選択肢」として位置づける。依頼者の言葉を借りれば「どちらが先だったのか」という
問いも込みで、次回以降に相談しながら進める。
